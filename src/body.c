                              ...
                              float            *force_y)
{
    ...
    *force_y = 0.0;
    ...
    __m256 bodies_x = _mm256_setr_ps(
        bodies[0].x, bodies[1].x, bodies[2].x, bodies[3].x,
        bodies[4].x, bodies[5].x, bodies[6].x, bodies[7].x
    );
    __m256 bodies_y = _mm256_setr_ps(
        bodies[0].y, bodies[1].y, bodies[2].y, bodies[3].y,
        bodies[4].y, bodies[5].y, bodies[6].y, bodies[7].y
    );
    __m256 bodies_mass = _mm256_setr_ps(
        bodies[0].mass, bodies[1].mass, bodies[2].mass, bodies[3].mass,
        bodies[4].mass, bodies[5].mass, bodies[6].mass, bodies[7].mass
    );
    ...
    __m256 distance_square = _mm256_add_ps(_mm256_mul_ps(distance_x, distance_x), _mm256_mul_ps(distance_y, distance_y));
    __m256 force = _mm256_div_ps(_mm256_mul_ps(_mm256_mul_ps(dest_mass, bodies_mass), m_gravity), distance_square);

    __m256 dx = _mm256_sub_ps(dest_x, bodies_x);
    __m256 dy = _mm256_sub_ps(dest_y, bodies_y);

    __m256 magnitude_inverse = _mm256_rsqrt_ps(_mm256_add_ps(_mm256_mul_ps(dx, dx), _mm256_mul_ps(dy, dy)));
    // __m256 magnitude_inverse = rsqrt_avx2(...);
